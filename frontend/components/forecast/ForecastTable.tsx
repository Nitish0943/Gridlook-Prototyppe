...
export default function ForecastTable({ locations, isLoading }: ForecastTableProps) {
  if (isLoading) {
    return (
  ...
  if (!locations || locations.length === 0) {
    return (
      <div className="w-full bg-slate-900/50 border border-slate-800 rounded-xl p-6 h-96 flex items-center justify-center">
        <p className="text-slate-500 font-mono text-sm">No forecast risk telemetry available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded-xl overflow-hidden shadow-2xl flex flex-col h-full">
      ...
      <div className="forecast-table-container flex-1">
        <table className="w-full min-w-[700px] text-left border-separate border-spacing-0">
          <thead>
            <tr className="text-[10px] uppercase font-black tracking-widest text-slate-500">
              ...
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Current Violations</th>
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Predicted (30d)</th>
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Growth %</th>
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Future Risk</th>
            </tr>
          </thead>
          <tbody className="text-xs font-mono">
            {locations.map((loc, idx) => {
              ...
              const badgeColors = CATEGORY_STYLES[loc.risk] || CATEGORY_STYLES.Low;
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4 border-b border-slate-900/40 text-slate-300">
                    {loc.current_violations.toLocaleString()}
                  </td>
                  <td className="p-4 border-b border-slate-900/40 text-slate-100 font-bold">
                    {loc.predicted_violations.toLocaleString()}
                  </td>
                  <td className={`p-4 border-b border-slate-900/40 font-bold ${loc.growth_rate >= 0 ? 'text-rose-455' : 'text-emerald-400'}`}>
                    {loc.growth_rate >= 0 ? '+' : ''}{loc.growth_rate}%
                  </td>
                  <td className="p-4 border-b border-slate-900/40">
                    <div className="flex items-center space-x-2">
                      <span className={`font-black text-sm ${scoreColor}`}>
                        {loc.future_risk_score}
                      </span>
                      <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded border tracking-wider font-mono ${badgeColors}`}>
                        {loc.risk}
                      </span>
                      {loc.risk === 'Critical' && <AlertCircle className="w-3.5 h-3.5 text-rose-500 shrink-0" />}
                    </div>
                  </td>
                </tr>
              );
