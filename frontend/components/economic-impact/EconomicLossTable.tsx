...
import { Landmark, AlertCircle, Activity } from 'lucide-react';
import { formatRupees } from './EconomicKPIs';

interface EconomicLossTableProps {
...
export default function EconomicLossTable({ locations, isLoading }: EconomicLossTableProps) {
  if (isLoading) {
    return (
  ...
  if (!locations || locations.length === 0) {
    return (
      <div className="w-full bg-slate-900/50 border border-slate-800 rounded-xl p-6 h-96 flex items-center justify-center">
        <p className="text-slate-500 font-mono text-sm">No economic impact data available.</p>
      </div>
    );
  }

  return (
    <div className="w-full bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded-xl overflow-hidden shadow-2xl flex flex-col h-full">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .economic-table-container::-webkit-scrollbar {
          width: 6px;
          height: 6px;
        }
        .economic-table-container::-webkit-scrollbar-track {
          background: rgba(2, 6, 23, 0.6);
        }
        .economic-table-container::-webkit-scrollbar-thumb {
          background: rgba(30, 41, 59, 0.8);
          border-radius: 4px;
          border: 1px solid rgba(51, 65, 85, 0.4);
        }
        .economic-table-container::-webkit-scrollbar-thumb:hover {
        ...
        <table className="w-full min-w-[700px] text-left border-separate border-spacing-0">
          <thead>
            <tr className="text-[10px] uppercase font-black tracking-widest text-slate-500">
              ...
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Daily Loss</th>
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Monthly Loss</th>
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Yearly Loss</th>
              <th className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-sm p-4 font-medium border-b border-slate-800">Severity</th>
            </tr>
          </thead>
          <tbody className="text-xs font-mono">
            {locations.map((loc, idx) => {
              let scoreColor = 'text-emerald-400';
              if (loc.category === 'Critical') scoreColor = 'text-rose-500';
              else if (loc.category === 'High') scoreColor = 'text-orange-400';
              else if (loc.category === 'Medium') scoreColor = 'text-yellow-400';

              const badgeColors = CATEGORY_STYLES[loc.category] || CATEGORY_STYLES.Low;
                ...
                >
                  ...
                  <td className="p-4 border-b border-slate-900/40">
                    <span className="text-slate-200 font-bold">
                      {formatRupees(loc.daily_loss)}
                    </span>
                  </td>
                  <td className="p-4 border-b border-slate-900/40">
                    <div className="flex items-center space-x-2">
                      <span className={`font-black text-sm ${scoreColor}`}>
                        {formatRupees(loc.monthly_loss)}
                      </span>
                      {loc.category === 'Critical' && <AlertCircle className="w-3.5 h-3.5 text-rose-500" />}
                    </div>
                  </td>
                  <td className="p-4 text-slate-300 border-b border-slate-900/40">
                    {formatRupees(loc.yearly_loss)}
                  </td>
                  <td className="p-4 border-b border-slate-900/40">
