...
export default function EconomicMap({ mapData, isLoading }: EconomicMapProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="w-full h-full bg-white border border-slate-200 rounded-xl overflow-hidden relative shadow-md flex flex-col min-h-[450px]">
      ...
      >
        ...
        {locations.map((loc) => {
          const color = CATEGORY_COLORS[loc.category] || CATEGORY_COLORS.Low;
          return (
            <CircleMarker
              key={loc.id}
              ...
              <Popup className="custom-leaflet-popup">
                <div style={{ minWidth: 220, color: '#1e293b', fontFamily: 'var(--font-inter), sans-serif', fontSize: '11px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ color: '#475569' }}>Daily Loss:</span>
                    <span style={{ fontWeight: 700, color: '#1e293b' }}>{formatRupees(loc.daily_loss)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ color: '#475569' }}>Monthly Loss:</span>
                    <span style={{ fontWeight: 700, color: '#1e293b' }}>{formatRupees(loc.monthly_loss)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ color: '#475569' }}>Yearly Loss:</span>
                    <span style={{ fontWeight: 700, color: '#1e293b' }}>{formatRupees(loc.yearly_loss)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ color: '#475569' }}>Economic Severity:</span>
                    <span style={{
                      fontWeight: 700,
                      fontSize: '9px',
                      padding: '1px 5px',
